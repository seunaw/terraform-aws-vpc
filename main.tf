...
resource "aws_nat_gateway" "this" {
  ...
  # @Swright
  subnet_id = !var.subnet_with_names ? element(aws_subnet.public.*.id,var.single_nat_gateway ? 0 : count.index) : element( aws_subnet.public_subnets_with_names.*.id,var.single_nat_gateway ? 0 : count.index)
